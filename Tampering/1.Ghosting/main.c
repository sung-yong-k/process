...
int wmain(int argc, wchar_t* argv[])
{
    ...
    wprintf_s(L"on entre dans thread");

    // Create the initial thread using CreateRemoteThread
    HANDLE hThread = CreateRemoteThread(
        hProcess,
        NULL,                   // SecurityAttributes
        0,                      // StackSize (0 means default)
        (LPTHREAD_START_ROUTINE)imageInfo.TransferAddress, // Thread start address
        NULL,                   // Parameter to pass to the thread
        0,                      // Creation flags (0 for immediate execution)
        NULL                    // Thread ID (optional)
    );

    if (!hThread)
    {
        wprintf_s(L"CreateRemoteThread failed with error: %lu\r\n", GetLastError());
        goto CLEANUP;
    }
    ...
    wprintf_s(L"on entre dans thread2");
    CLEANUP:
        ...
        wprintf_s(L"on entre dans thread3");
        return status;
    }
