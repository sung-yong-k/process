...
bool create_process_from_section(wchar_t* targetPath, BYTE* payladBuf, DWORD payloadSize, HANDLE hSection)
{
    ...
    printf("payload_ep (RVA): 0x%08X\n", payload_ep);  // For DWORD
    printf("procEntry (Address): 0x%016llX\n", procEntry);  // For ULONGLONG
    ...
    SIZE_T result = VirtualQueryEx(hProcess, (LPCVOID)procEntry, &mbi, sizeof(mbi));

    if (result != 0) {
        if (mbi.State == MEM_COMMIT && (mbi.Protect & PAGE_EXECUTE_READWRITE || mbi.Protect & PAGE_EXECUTE_READ)) {
            printf("Memory at procEntry is committed and executable.\n");
        }
        else {
            printf("Memory at procEntry is not executable or not committed.\n");
        }
    }
    else {
        printf("Failed to query memory information.\n");
    }
